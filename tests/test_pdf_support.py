...
class PdfExtractionTests(unittest.TestCase):
    ...
    def test_footer_watermark_is_visible_but_not_selectable(self):
        document = fitz.open()
        page = document.new_page(width=420, height=300)
        page.insert_text((40, 70), "Selectable lesson content", fontsize=12)
        page.insert_text((40, 290), "CONFIDENTIAL WATERMARK", fontsize=10)
        data = document.tobytes()
        document.close()

        result = extract_pdf_document(data, "watermarked.pdf")
        page_data = result["pages"][0]
        selectable_words = " ".join(word["text"] for word in page_data["words"])

        self.assertIn("Selectable lesson content", selectable_words)
        self.assertNotIn("CONFIDENTIAL WATERMARK", selectable_words)
        self.assertNotIn("CONFIDENTIAL WATERMARK", page_data["selection_text"])
